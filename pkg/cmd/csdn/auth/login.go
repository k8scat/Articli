...
var (
	...
	loginCmd = &cobra.Command{
		Use:   "login",
		Short: "Authenticate with csdn.net",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cookieStdin {
				...
				if client != nil {
					for {
						bo.Printf("? You're already logged in as '%s'. Do you want to re-login? ", client.AuthInfo.Basic.Nickname)
						wo.Print("(y/N) ")
			...
			client, err := csdnsdk.NewClient(cookie)
			if err != nil {
			...
			cfg.Platforms.CSDN.Cookie = cookie
